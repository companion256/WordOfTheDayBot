...
def makeRelayManager(relays):
    ...
    for nostrRelay in relays:
        relaysLeftToAdd -= 1
        if relaysLeftToAdd <= 0:
            break
        if type(nostrRelay) is dict:
            newRelayManager.add_relay(
                url=nostrRelay["url"],
                read=nostrRelay["read"],
                write=nostrRelay["write"],
            )
        if type(nostrRelay) is str:
            newRelayManager.add_relay(url=nostrRelay)
    newRelayManager.open_connections(
...
def getNostrRelaysFromConfig(aConfig):
    ...
    if "relays" in aConfig:
        for relay in aConfig["relays"]:
            ...
            relayUrl = (
                relayUrl if str(relayUrl).startswith("wss://") else f"wss://{relayUrl}"
            )
    ...
    if "excludeFromDirectMessages" in config:
        ...
        for exclude in excludes:
            if exclude["npub"] == toUserNpubOrHex:
                logger.debug(
                    "Not sending direct message to excluded: {toUserNpubOrHex}"
                )
                return
            if exclude["npub"] == recipient_pubkey:
                logger.debug(
                    "Not sending direct message to excluded pubkey: {recipient_pubkey}"
                )
                return
    dm = EncryptedDirectMessage(
        recipient_pubkey=recipient_pubkey, cleartext_content=message
    )
...
def getProfile(pubkeyHex):
    ...
    for profile in _monitoredProfiles:
        if profile.public_key != pubkeyHex:
            ...
            continue
        if profile.created_at < created_at:
            continue
        if not isValidSignature(profile):
            continue
        try:
...
def authenticateRelays(theRelayManager, pk):
    if not theRelayManager.message_pool.has_auths():
        return False
    while theRelayManager.message_pool.has_auths():
        auth_msg = theRelayManager.message_pool.get_auth()
        logger.info(
            f"AUTH request received from {auth_msg.url} with challenge: {auth_msg.challenge}"
        )
...
def siftMessagePool():
    ...
    while _relayManager.message_pool.has_events():
        ...
        if subid.startswith("my_dms"):
            _directMessages.append(event_msg.event)
        elif subid.startswith("my_events"):
            _monitoredEvents.append(event_msg.event)
        elif subid.startswith("my_pubkeys"):
            _monitoredPubkeys.append(event_msg.event)
        elif subid.startswith("my_profiles"):
            _monitoredProfiles.append(event_msg.event)
        elif subid.startswith("my_eventbyid"):
            _monitoredEvent.append(event_msg.event)
        else:
            u = event_msg.url
            c = event_msg.event.content
            logger.debug(
                f"Unexpected event from relay {u} with subscription {subid}: {c}"
            )
    ...
    for relayConfig in _relayManager.relays.values():
        ...
        if found:
            ...
            if needToAdd:
                ...
                relayConfig.add_subscription(
                    id=subscription_events, filters=filters_events
                )
                added = True
            elif not hasEvent:
                relayConfig.update_subscription(
                    id=subscription_events, filters=filters_events
                )
                updated = True
        else:
    ...
    for eventReply in _monitoredEvents:
        ...
        if not isValidSignature(eventReply):
            ...
            for tagItem in eventReply.tags:
                if len(tagItem) < 2:
                    continue  # exclude tags without values
                if tagItem[0] != "e":
                    continue  # not event tag
...
def getEventsByPubkey(pubkeyHex):
    ...
    for relayConfig in _relayManager.relays.values():
        ...
        if found:
            ...
            if not hasPubkey:
                relayConfig.update_subscription(
                    id=subscription_pubkeys, filters=filters_pubkeys
                )
                updated = True
        else:
            filters_pubkeys = Filters(
                [
                    Filter(
                        authors=[pubkeyHex],
                        kinds=[EventKind.TEXT_NOTE],
...
def makeZapRequest(
    withPK, amountToZap, zapMessage, recipientPubkey, eventId, bech32lnurl
):
    ...
    for relay in relays:
        ...
        if type(relay) is dict:
            ...
            if canread and "url" in relay:
                relays4zapReceipt.append(relay["url"])
        relaysLeftToAdd -= 1
    ...
    for k, v in lightningIdCache.items():
        ...
        if v["created_at"] > t - 86400:
            ...
            if lightningId is not None:
                name = (
                    v["name"] if ("name" in v and v["name"] is not None) else "no name"
                )
...
def makeLightningIdFromLNURL(lnurl):
    ...
    try:
        ...
        logger.warning(
            f"Could not decode lnurl ({lnurl}) to a lightning identity: {str(err)}"
        )
    return lightningId
...
def isValidLightningId(lightningId):
    ...
    if len(identityParts) != 2:
        return (
            False,
            f"Lightning address {lightningId} is invalid - not in username@domain format",
        )
    return True, None
...
def validateLNURLPayInfo(lnurlPayInfo, lnurlp, lightningId, name, amount, pubkey):
    ...
    if "nostrPubkey" not in lnurlPayInfo:
        logger.warning(
            f"LN Provider for {lightningId} does not have nostrPubkey. Publisher of receipt could be anyone"
        )
    if not all(k in lnurlPayInfo for k in ("callback", "minSendable", "maxSendable")):
        logger.debug(
            f"LN Provider for {lightningId} does not have proper callback, minSendable, or maxSendable info."
        )
        userMessage = (
            f"LN Provider for {lightningId} does not provide expected response format"
        )
        return callback, bech32lnurl, userMessage
    ...
    if (amount * 1000) < minSendable:
        logger.debug(
            f"LN Provider for {lightningId} does not allow zaps less than {minSendable} msat."
        )
        userMessage = f"LN Provider for {lightningId} requires {minSendable} msats minimum to be zapped"
        return callback, bech32lnurl, userMessage
    if (amount * 1000) > maxSendable:
        logger.debug(
            f"LN Provider for {lightningId} does not allow zaps greater than {maxSendable} msat."
        )
    ...
    if callback is None:
        ...
        logger.debug(
            f"LN Provider callback for {lightningId} is on the denyProviders list and cannot be zapped at this time ({name} with pubkey: {pubkey})"
        )
