...
def geturl(useTor=True, url=None, defaultResponse="{}", headers={}, description=None):
    try:
        ...
        logger.warning(
            f"Error getting data from LN URL Provider from url ({url}): {str(e)}"
        )
...
def isValidInvoiceResponse(invoiceResponse):
    if "status" in invoiceResponse:
        if invoiceResponse["status"] == "ERROR":
            ...
            if "reason" in invoiceResponse:
                errReason = invoiceResponse["reason"]
            logger.warning(f"Invoice request error: {errReason}")
            return False
    if "pr" not in invoiceResponse:
        return False
    return True
...
def isValidInvoiceAmount(decodedInvoice, amountToZap):
    ...
    if num_satoshis != amountToZap:
        logger.warning(
            f"Invoice amount ({num_satoshis}) does not match requested amount ({amountToZap}) to zap"
        )
        return False
    num_msat = int(decodedInvoice["num_msat"])
    if num_msat != amountMillisatoshi:
        logger.warning(
            f"Invoice amount of msats ({num_msat}) does not match requested amount ({amountMillisatoshi}) to zap"
        )
        return False
    return True
