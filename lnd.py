...
def getLNDServerConfig():
    ...
    if activeConfig is not None:
        return activeConfig
    if not all(k in config for k in ("activeServer", "servers")):
        activeConfig = dict(config)
        return activeConfig
    activeServerId = config["activeServer"]
    if activeServerId is None:
        activeConfig = dict(config)
        return activeConfig
    servers = config["servers"]
    if activeServerId not in servers:
        activeConfig = dict(config)
        return activeConfig
    activeServer = servers[activeServerId]
    if all(k in activeServer for k in ("address", "port", "macaroon")):
        activeConfig = dict(activeServer)
        return activeConfig
    return config
...
def getLNDHeaders():
    ...
    headers = {"Grpc-Metadata-macaroon": serverMacaroon, "Connection": "close"}
    return headers
...
def getLNDTimeouts():
    ...
    connectTimeout = 5
    readTimeout = 30
    if "connectTimeout" in lndServerConfig:
        connectTimeout = lndServerConfig["connectTimeout"]
    if "readTimeout" in lndServerConfig:
        readTimeout = lndServerConfig["readTimeout"]
...
def restLndGET(suffix):
    try:
        ...
        response = requests.get(
            url=url, timeout=timeout, proxies=proxies, headers=headers, verify=False
        )
    ...
    if result is not None:
        if "message" in result:
            ...
            if message == "permission denied":
                logger.warning(
                    "LND reports Permission Denied to create invoice. Check macaroon permissions"
                )
                return None
    return result
...
def trackPayment(paymentHash):
    ...
    try:
        ...
        Exception
    ) as rte:  # (ConnectionError, TimeoutError, ReadTimeoutError) as rte:
...
def payInvoice(paymentRequest):
    ...
    feeLimit = 2
    paymentTimeout = 30
    if "feeLimit" in lndServerConfig:
        lndServerConfig["feeLimit"]
    if "paymentTimeout" in lndServerConfig:
        paymentTimeout = lndServerConfig["paymentTimeout"]
    ...
    try:
        for raw_response in r.iter_lines():
            ...
            if "payment_hash" in json_response:
                ...
                if payment_hash is not None:
                    if new_payment_hash != payment_hash:
                        logger.info(
                            f"PAYMENT HASH changed from {payment_hash} to {new_payment_hash}"
                        )
            ...
            if resultStatus == "SUCCEEDED":
                logger.debug(
                    f" - {resultStatus}, routing fee paid: {resultFeeMSat} msat"
                )
        ...
        try:
            ...
            logger.warning(
                f"Error closing connection after exception in payInvoice: {str(e)}"
            )
        if json_response is not None:
            logger.debug(json_response)
...
def checkInvoices():
    ...
    for invoice in _invoices:
        ...
        if status is None:
            ...
            logger.warning(
                f"invoice for {npub} has no status. payment_hash for lookup is {payment_hash}"
            )
            continue
        if "STATE" not in status and "state" not in status:
            openInvoices.append(invoice)  # could be temp error?
            logger.warning(
                f"invoice for {npub} does not have a STATE. payment_hash for lookup is {payment_hash}"
            )
        ...
        if state == "OPEN":
            ...
            logger.warning(
                f"invoice for {npub} has unrecognized state ({state}). payment_hash for lookup is {payment_hash}"
            )
            logger.warning(f"response of lookupInvoice: ")
            logger.warning(json.dumps(obj=status, indent=2))
...
def recordPaymentDestination(decodedInvoice):
    if not all(
        k in decodedInvoice for k in ("destination", "num_satoshis", "num_msat")
    ):
        return
    ...
    d = {}
    if diso in pddata.keys():
        d = pddata[diso]
    dpk = {"qty": 0, "amount": 0}
    if destination_pubkey in d.keys():
        dpk = d[destination_pubkey]
    dpk["qty"] = dpk["qty"] + 1
