...
def getTimes(aDate=None):
    theDate = aDate
    if aDate is None:
        theDate = datetime.datetime.now()  # utcnow()
    secTime = int(theDate.timestamp())
    isoTime = theDate.utcfromtimestamp(theDate.timestamp()).isoformat(
        timespec="seconds"
    )
        ...
        if type(list) is dict:
            ...
            if type(v) is dict:
                if fieldname in v:
                    if value is None:
                        count += 1
                    elif v[fieldname] == value:
                        count += 1
        if type(item) is dict:
            if fieldname in item:
                if value is None:
                    count += 1
                elif item[fieldname] == value:
                    count += 1
